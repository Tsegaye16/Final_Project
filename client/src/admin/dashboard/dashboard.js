...
function AdminDashdoard() {
  ...
  const [users, setUsers] = useState([]);;
    //const {id} = useParams()
    useEffect(() => {
        axios.get("http://localhost:8800/admin/viewRecent/")
  ...
  const handleDeleteConfirm = () => {
    axios.delete(`http://localhost:8800/admin/deleteUser/${deleteCandidate.user_id}`)
  .then((resp) => {
    // Handle success
       ...
       <table>
                        ...
                        <tbody>
                            {users && users.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan="11" style={{ textAlign: 'center' }}>No Recently registered userr</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
