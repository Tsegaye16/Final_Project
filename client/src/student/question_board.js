...
function QuizQuestions() {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:8800/student/startQuiz", { quiz_id: quizId })
      .then((response) => {
  ...
  const handleSave = (questionId) => {
    const selectedQuestion = questionsWithChoices.find(
      (question) => question.question_id === questionId
    );
    console.log("Selected question", selectedQuestion);
    console.log("good", selectedOptions[questionId]);
    const selectedChoice = selectedQuestion.choices.find(
      (choice) => choice.choice_id === selectedOptions[questionId]
    );
    ...
    const correctAnswer = selectedQuestion.choices.find(
  ...
  return (
    <div style={{ display: "flex", flexDirection: "row" }}>
            ...
            questionsWithChoices.map((question, questionIndex) => (
              ...
              >
                ...
                <Typography
                  ...
                  style={{
                    marginLeft: "10px",
                    color: resultMessages[question.question_id]
                      ? resultMessages[question.question_id].includes("Great")
                        ? "green"
                        : "red"
                      : "inherit",
                  }}
                ...
                {selectedOptions[question.question_id] && (
                  ...
                  >
                    <Button
                      ...
                      disabled={disabledOptions[question.question_id]}
                    >
                      Save
                    </Button>
                  </Box>
      ...
      <div
        style={{ position: "fixed", right: "20px", top: "20px", zIndex: 1000 }}
      >
        <Container
          maxWidth="md"
          ...
          {Object.keys(selectedOptions).length > 0 && (
            <>
              ...
              <Typography variant="body1" gutterBottom>
                You lose:{" "}
                {(
                  (Object.values(resultMessages).filter((message) =>
                    message.includes("You missed it")
                  ).length /
                    Object.keys(selectedOptions).length) *
                  100
                ).toFixed(2)}
              ...
              <Typography variant="body1" gutterBottom>
                ...
                {(
                  (Object.values(resultMessages).filter((message) =>
                    message.includes("Great job! Go ahead")
                  ).length /
                    Object.keys(selectedOptions).length) *
                  100
                ).toFixed(2)}
      ...
      <div
        style={{ position: "fixed", left: "20px", top: "20px", zIndex: 1000 }}
      >
