...
function Quiz() {
  ...
  const handleConfirmationDialogConfirm = () => {
    ...
    axios.post('http://localhost:8800/student/startQuiz', { quiz_id: selectedQuizId })
      .then(response => {
          ...
          <Card className="quiz-card">
            <CardContent>
              <Typography variant="h4" gutterBottom>
                The quiz {quiz.quiz_name} has {quiz.total_number_of_questions} questions
              </Typography>
              <List>
                <ListItem>
                  <Typography variant="body1" paragraph>
                    {quiz.total_number_of_easy} easy questions, contains {quiz.total_marks_of_easy} marks
                  </Typography>
                </ListItem>
                <ListItem>
                  <Typography variant="body1" paragraph>
                    {quiz.total_number_of_medium} medium questions, contains {quiz.total_marks_of_medium} marks
                  </Typography>
                </ListItem>
                <ListItem>
                  <Typography variant="body1">
                    {quiz.total_number_of_hard} hard questions, contains {quiz.total_marks_of_hard} marks
                  </Typography>
                </ListItem>
              </List>
              <Button variant="contained" color="primary" onClick={() => handleAttemptClick(quiz.quiz_id)}>
                Start
              </Button>
            </CardContent>
          </Card>
