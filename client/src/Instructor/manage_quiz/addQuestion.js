...
const AddQuestion = ({ cancel, quiz_name, quiz_id }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (
      difficultyLevels.some((level) => !level) ||
      marks.some((mark) => mark === 0)
    ) {
      toast.error(
        "Please select difficulty level and provide marks for all questions"
      );
      return;
    }
    ...
    try {
      await axios.post(
        "http://localhost:8800/instructors/addQuestion",
        payload
      );
  ...
  return (
    <div className="add-question-container">
            ...
            <Grid item xs={12} sm={4}>
              ...
              >
                ...
                {questions[index].choices.map((choice, choiceIndex) => (
                  <MenuItem key={choiceIndex} value={choiceIndex}>{`Choice ${
                    choiceIndex + 1
                  }`}</MenuItem>
                ))}
              </Select>
      ...
      <div className="button-cont">
        <Button
          variant="contained"
          className={`save ${isSaveDisabled ? "disable" : ""}`}
