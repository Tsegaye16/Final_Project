...
function Stack() {
  ...
  const [stackHeights, setStackHeights] = useState(Array.from({ length: queueSize }, () => 0));
  ...
  return (
    <div className='main-stack'>
      ...
      <div className='input-container'>
        ...
        <button className='stack-button' onClick={handlePush}>Push</button>
      </div>
      <div className='input-container'>
        <button className='stack-button' onClick={handlePop}>Pop</button>
      </div>
    ...
    >
      ...
      <Box className="numbers-outside" sx={{ display: 'flex', flexDirection:'column' }}>
        {Array.from({ length: queueSize }, (_, i) => (
          <div key={i} style={{borderBottom:'1px black solid', height:'50px'}}>{queueSize - i - 1}</div>
        ))}
      </Box>
      </div>
