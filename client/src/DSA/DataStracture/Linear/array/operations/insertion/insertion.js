...
function Insertion() {
  ...
  const renderTableCells = () => {
    return Array(arrayLength).fill(null).map((_, index) => {
      ...
      const isCurrentCellFree = !isShifting && !highlightedCells.includes(index + 1);
      ...
      const boxStyle = {
        ...
        backgroundColor: isShifting ? '#f39c12' : (isCurrentCellFree ? 'transparent' : '#ecf0f1'),
        transform: isShifting ? `translateX(${index * 30}px)` : 'none',
        transition: isShifting ? 'transform 0.5s ease-in-out' : 'none',
      };

      return (
        <TableCell key={index} style={{ borderRight: '1px solid #000', width: '30px' }}>
    ...
    <Box p={2}>
      <Box display="flex" justifyContent="space-between">
        ...
        <Box display="flex" flexDirection="column">
          <Button onClick={handleInsertBeginning} variant='contained' color='secondary' style={{ marginLeft: '8px', marginBottom: "20px", backgroundColor: '#2c3e50' }}>
            At the Beginning
          </Button>
        ...
        <Box>
          ...
          <Button variant='contained' color='secondary' style={{ marginLeft: '8px', backgroundColor: '#2c3e50' }}>
            At the Index
          </Button>
        </Box>
      </Box>
